...
void cross_validation(Data *instance, int folds, char *labels[], int epochs, float learning_rate); // k fold stratified cross validation
int test_network(Network *nw, Data *test_data, int test_size, char *labels[]); // test network

int main(int argc, char* argv[]){
	if (argc != 4)
	{
		printf("Error usage! Required 3 arguments: folds, epochs, learning_rate\n");
		return 1;
	}
	// Open input file
	FILE *fp;
	...
	//print_network(&nw);
	// Cross Validation
	cross_validation(instance, num_folds, labels, epochs, learning_rate);
	for (int i = 0; i < 2; i++)
		free(labels[i]);	
}
...
void cross_validation(Data *instance, int folds, char *labels[], int epochs, float learning_rate)
{
	...
	{	
		...
		double elapsed = (end.tv_sec - start.tv_sec)*1000 + (double)(end.tv_usec - start.tv_usec)/1000; // elapsed time in msec
		total_time += elapsed;		
	}
	//Deallocate memory
	for (int i = 0; i < folds; i++)
...
void initialize_network(Network *nw)
{
	...
	for (int i = 0; i < NUM_ATTR; i++) 
	{
		...
		//random_num = 0.1; // fix to a constant value to compare with OMP result. Commented out when running actual training
	...
	for (int i = 0; i < NUM_ATTR; i++)
	{
		for (int j = 0; j < NUM_ATTR; j++)
		{
			...
			//random_num = 0.2; // used when validate OMP result only
	...
	{
		random_num = rand()/(float)RAND_MAX;
		//random_num = 0.3; // used when validate OMP result only
		random_num = -1 + random_num*2;
		nw->b[i] = random_num;
	}
	//random_num = 0.4; // used when validate OMP result only
	random_num = rand()/(float)RAND_MAX; 
	random_num = -1 + random_num*2; 
	nw->c = random_num;
}
