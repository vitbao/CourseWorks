...
NetworkValue feed_forward(Network *nw, float* x, int num_threads); // compute values of the network
...
NetworkError backpropagation_error(Data *instance, NetworkValue *nwValue, Network *nw, char *labels[], int num_threads); // calculate errors of the network using back propagation algorithm
void update_network(Network *nw, NetworkError *nwError, float learning_rate, int num_threads); // Update network using computed network error
...
void train_network(Network *nw, Data *train_data, int epochs, int train_size, char * labels[], float learning_rate, int num_threads); //training network
...
int test_network(Network *nw, Data *test_data, int test_size, char *labels[], int num_threads); // test network
...
void cross_validation(Data *instance, int folds, char *labels[], int epochs, float learning_rate, int num_threads)
{
	...
	{	
		if (folds == 1)
		{
			...
			// Train network
			train_network(&nw, train_set, epochs, train_size, labels, learning_rate, num_threads);
			// Test network
			accuracy = test_network(&nw, test_set, test_size, labels, num_threads);
			//printf("%d\n", accuracy);
			// End timing
	...
	printf("%d%s%d%s%d%s%f\n", folds, ", ", epochs, ", ", num_threads, ", ",  total_time/10);
	//Deallocate memory
	for (int i = 0; i < folds; i++)
	{
...
void train_network(Network *nw, Data *instance, int epochs, int train_size, char *labels[], float learning_rate, int num_threads)
{
	for (int i = 0; i < epochs; i++)
	{
		...
		{
			nwValue = feed_forward(nw, instance[j].attr_values, num_threads);
			nwError = backpropagation_error(&instance[j], &nwValue, nw, labels, num_threads);
			update_network(nw, &nwError, learning_rate, num_threads);
		}	
	}
}
...
NetworkValue feed_forward(Network *nw, float* instance, int num_threads)
{
	NetworkValue nwValue;
	omp_set_num_threads(num_threads);
...
NetworkError backpropagation_error(Data *instance, NetworkValue *nwValue, Network *nw, char* labels[], int num_threads)
{
	NetworkError nwError;
...
void update_network(Network *nw, NetworkError *nwError, float learning_rate, int num_threads)
{
...
int test_network(Network *nw, Data *test_data, int test_size, char *labels[], int num_threads)
{
	int num_correct = 0;
	NetworkValue nwValue;
	...
	{
		nwValue = feed_forward(nw, test_data[i].attr_values, num_threads);
		...
		if (strcmp(predicted_label, actual_label) == 0)
			num_correct += 1;
		//printf("%d%s%s%s%s\n", test_data[i].index, ", ", actual_label, ", ", predicted_label);
		free(actual_label);
